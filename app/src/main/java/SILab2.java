...
public class SILab2 {
    public static double checkCart(List<Item> allItems, String cardNumber){
        if (allItems == null){// 1
            throw new RuntimeException("allItems list can't be null!"); //2  frlat
        }
        ...
        for (int i = 0; i < allItems.size(); i++){ // 4
            ...
            if (item.getName() == null || item.getName().length() == 0){ // 6
                throw new RuntimeException("Invalid item!"); // 7 frlat
            }
        ...
        if (cardNumber != null && cardNumber.length() == 16) { // 13 //1234567891011121
            ...
            for (int j = 0; j < cardNumber.length(); j++) { // 16
                ...
                if (allowed.indexOf(c) == -1) { //18
                    throw new RuntimeException("Invalid character in card number!"); //19 frlat
                }
            }
        }
        else{
